...
class Machines:
    ...
    def startListen(self):
        ...
        self.udp_socket.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF,
                                   5 * 1024 * 1024)
    ...
    def send(self, msg, ip="<broadcast>"):
        ...
        _udp_broadcast_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR,
                                         1)
        ...
        if isinstance(ip, (tuple, list)):
            ...
            _udp_broadcast_socket.setsockopt(socket.SOL_SOCKET,
                                             socket.SO_BROADCAST, 1)
            _udp_broadcast_socket.sendto(msg, ('255.255.255.255', 20086))
        else:
            _udp_broadcast_socket.sendto(msg, (ip, 20086))

    def sendAndReceive(self, msg, ip="<broadcast>", trycount=0, timeout=1,
                       callback=None):
        """
        发送消息，并等待消息返回
        """
    ...
    def queryAllInterfaces(self, ip="<broadcast>", trycount=1, timeout=1):
        ...
        """
        self.reset()
        nameLen = len(self.username) + 1

        msg = MessageStream.MessageStreamWriter(
            MachineInterface_onQueryAllInterfaceInfos)
    ...
    def queryMachines(self, ip="<broadcast>", trycount=1, timeout=1):
        ...
        msg = MessageStream.MessageStreamWriter(
            MachineInterface_onQueryMachines)
        msg.writeInt32(self.uid)
        msg.writeString(self.username)
        msg.writeUint16(socket.htons(self.replyPort))  # reply port
    ...
    def startServer(self, componentType, cid, gus, targetIP, kbe_root,
                    kbe_res_path, kbe_bin_path, trycount=1, timeout=1):
        """
        """
    ...
    def stopServer(self, componentType, componentID=0, targetIP="<broadcast>",
                   trycount=1, timeout=1):
        """
        """
    ...
    def killServer(self, componentType, componentID=0, targetIP="<broadcast>",
                   trycount=1, timeout=1):
        """
        """
    ...
    def parseQueryData(self, recvData):
        ...
        # 如果pid与machineID相等，表明这个是machine进程
    ...
    def makeCID(self, componentType):
        ...
        cid = "%02i%05i%05i%04i" % (
        componentType, t, self.cidRand, self.ct2cid[componentType])
        return int(cid)
