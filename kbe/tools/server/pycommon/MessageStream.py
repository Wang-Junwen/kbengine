...
class MessageStreamWriter(object):
    ...
    def writeBlob(self, val):
        ...
        assert isinstance(val,
                          bytes), "value type not match! current val type = %s" % type(
            val)
    ...
    def writeString(self, val):
        """
        """
        assert isinstance(val, (bytes, str,
                                unicode)), "value type not match! current val type = %s" % type(
            val)
        if isinstance(val, bytes):
            self.stream.write(
                struct.pack("=%ss" % (len(val) + 1), val))  # 加1是为了有空终结符
        else:
