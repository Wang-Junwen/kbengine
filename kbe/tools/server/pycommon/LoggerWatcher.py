...
class LoggerWatcher:
    ...
    def registerToLogger(self, uid):
        ...
        msg.write(struct.pack("=H", struct.calcsize(
            "=iIiiccB" + "i" * Define.COMPONENT_END_TYPE + "BB")))  # package len
        ...
        msg.write(struct.pack("=iicc", 0, 0, '\0'.encode(),
                              '\0'.encode()))  # globalOrder, groupOrder, date, keyStr
        msg.write(struct.pack("=B",
                              Define.COMPONENT_END_TYPE))  # component type filter count
        msg.write(struct.pack("=" + "i" * Define.COMPONENT_END_TYPE, *list(
            range(Define.COMPONENT_END_TYPE))))  # component type filter
        msg.write(struct.pack("=BB", 0, 1))  # isfind, first
        self.socket.sendall(msg.getvalue())

    def registerToLoggerForWeb(self, uid, components_check, logtype,
                               globalOrder, groupOrder, searchDate, keystr):
        """
        向logger注册
        """
        ...
        msg.write(struct.pack("=H", Logger_registerLogWatcher))  # command
        msg.write(struct.pack("=H", struct.calcsize(
            "=iIiiccB" + "i" * Define.COMPONENT_END_TYPE + "BB") + len(
            searchDate.encode()) + len(keystr.encode())))  # package len
        msg.write(struct.pack("=i", uid))
        msg.write(struct.pack("=I", logtype))  # logtypes filter
        # msg.write( struct.pack("=I",KBELOG_WARNING ) ) # logtypes filter
        msg.write(
            struct.pack("=ii" + d1 + "sc" + d2 + "sc", globalOrder, groupOrder,
                        searchDate.encode(), '\0'.encode(), keystr.encode(),
                        '\0'.encode()))  # globalOrder, groupOrder, date, keyStr
        msg.write(struct.pack("=B",
                              Define.COMPONENT_END_TYPE))  # component type filter count
        msg.write(struct.pack("=" + "i" * Define.COMPONENT_END_TYPE, *list(
            list(components_check))))  # component type filter
        msg.write(struct.pack("=BB", 0, 1))  # isfind, first
        self.socket.sendall(msg.getvalue())
    ...
    def sendActiveTick(self):
        ...
        msg.write(struct.pack("=iQ", Define.WATCHER_TYPE,
                              0))  # componentType, componentID
        self.socket.sendall(msg.getvalue())
    ...
    def sendLog(self, uid, type, logStr):
        ...
        msg.write(struct.pack("=H", struct.calcsize(
            "=iIiQiiqII") + logSize))  # package len
        msg.write(struct.pack("=i", uid))
        msg.write(struct.pack("=I", logName2type[type]))  # logtype
        msg.write(struct.pack("=i", Define.WATCHER_TYPE))
        msg.write(struct.pack("=Qii", 0, 0,
                              0))  # componentID, globalOrder, groupOrder
    ...
    def receiveLog(self, callbackFunc, loop=False):
        ...
        while True:
            ...
            if rlist:
                ...
                if len(msg) == 0:
                    print(
                        "Receive 0 bytes, over! fileno '%s'" % self.socket.fileno())
                    return
