...
class ClusterConsoleHandler(ClusterControllerHandler):
    ...
    def do(self):
        ...
        for machineID in self.interfaces_groups:
            ...
            for info in infos:
                if info.componentName + str(
                        info.groupOrderID) == self.consoleType or \
                        info.componentName + str(
                    "%02d" % (info.groupOrderID)) == self.consoleType:
    ...
    def do(self):
        ...
        for machineID in self.interfaces_groups:
            ...
            if len(infos) > 0:
                info = infos.pop(0)
                print(
                    "[%s: %%CPU:%.2f, %%MEM:%.2f, %%pCPU:%.2f, pMem:%.2fm, totalMem=%.2fm/%.2fm, addr=%s]" % \
                    (info.componentName, info.cpu, info.mem,
                     info.extradata2 / 100.0, info.usedmem / 1024.0 / 1024.0,
                     info.extradata1 / 1024.0 / 1024.0,
                     info.extradata / 1024.0 / 1024.0, info.intaddr))
            ...
            for info in infos:
                if info.componentType == BASEAPP_TYPE:
                    print(
                        "|-%12s\t%i\t%i\t%i\t%i\t%.2f\t%.2f\t%.2fm\tbases=%i\t\tclients=%i\tproxices=%i" % \
                        ((info.componentName + ("%.2i" % info.groupOrderID)),
                         info.componentID, info.uid, info.pid,
                         info.globalOrderID,
                         info.cpu, info.mem, info.usedmem / 1024.0 / 1024.0,
                         info.extradata, info.extradata1, info.extradata2))
                    ...
                    print(
                        "|-%12s\t%i\t%i\t%i\t%i\t%.2f\t%.2f\t%.2fm\tentities=%i\tcells=%i\t\t%i" % \
                        ((info.componentName + ("%.2i" % info.groupOrderID)),
                         info.componentID, info.uid, info.pid,
                         info.globalOrderID,
                         info.cpu, info.mem, info.usedmem / 1024.0 / 1024.0,
                         info.extradata, info.extradata1, 0))

                    numEntities += info.extradata
                    numCells += info.extradata1
                elif info.componentType == LOGINAPP_TYPE:
                    print(
                        "|-%12s\t%i\t%i\t%i\t%i\t%.2f\t%.2f\t%.2fm\t%i\t\t%i\t\t%i" % \
                        ((info.componentName + ("%.2i" % info.groupOrderID)),
                         info.componentID, info.uid, info.pid,
                         info.globalOrderID, info.cpu,
                         info.mem, info.usedmem / 1024.0 / 1024.0, 0, 0, 0))
                else:
                    print(
                        "|-%12s\t%i\t%i\t%i\t%i\t%.2f\t%.2f\t%.2fm\t%i\t\t%i\t\t%i" % \
                        (info.componentName, info.componentID, info.uid,
                         info.pid, info.globalOrderID, info.cpu,
                         info.mem, info.usedmem / 1024.0 / 1024.0, 0, 0, 0))

        print('-----------------------------------------------------')
        print(
            "machines: %i, components=%i, numBases=%i, numProxices=%i, numClients=%i, numEntities=%i, numCells=%i." % \
            (len(self.interfaces_groups), numComponent, numBases, numProxices,
             numClients, numEntities, numCells))


class ClusterStartHandler(ClusterControllerHandler):
    def __init__(self, uid, startTemplate, machineIP, cid, gus, kbe_root,
                 kbe_res_path, kbe_bin_path):
        ClusterControllerHandler.__init__(self, uid)
    ...
    def do(self):
        ...
        for ctype in self.startTemplate:
            ...
            self.startServer(COMPONENT_NAME2TYPE[ctype], self.cid, self.gus,
                             self.machineIP, self.kbe_root, self.kbe_res_path,
                             self.kbe_bin_path)
        ...
        while (True):
            ...
            self.queryAllInterfaces(MACHINES_ADDRESS,
                                    MACHINES_QUERY_ATTEMPT_COUNT,
                                    MACHINES_QUERY_WAIT_TIME)
        ...
        for ctype in self.interfaces:
            ...
            for info in infos:
                if info.uid == self.uid:
                    clist.append(info.componentID)
            print(
                "\t\t%s : %i\t%s" % (COMPONENT_NAME[ctype], len(clist), clist))
    ...
    def do(self):
        ...
        for machineID, infos in self.interfaces_groups.items():
            for info in infos:
                ...
                cper.set(COMPONENT_NAME[info.componentType],
                         "item_%s" % t2c[info.componentType], v)
    ...
    def do(self):
        ...
        for ct in SIGNLE_CT:
            ...
            if cper.has_option(secName, optName):
                ...
                print("run '%s' in '%s', uid = %s, cid = %s, gus = %s" % (
                secName, targetIP, self.uid, cid, gus))
        ...
        while True:
            ...
            for i in range(len(expectCount)):
                if expectCount[i] <= 0:
                    continue
                print("\t%11s: %s/%s" % (
                COMPONENT_NAME[i], currentCount[i], expectCount[i]))
...
class ClusterStartServerHandler(ClusterControllerHandler):
    ...
    def do(self):
        ...
        for ct in self.VALIDATE_CT:
            ...
            gus = self.makeGUS(ct)
            print("run '%s' in '%s', uid = %s, cid = %s, gus = %s" % (
            secName, self.machineIP, self.uid, cid, gus))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2:
        ...
        if cmdType == "start":
            ...
            if len(sys.argv) > 3:
                if sys.argv[3].isdigit():
                    ...
                    print(
                        "syntax: cluster_controller.py start [machine-ip] [user-id]")
            ...
            if len(sys.argv) < 6:
                print(
                    "syntax: cluster_controller.py startprocess componentName cid gus machine-ip [user-id]")
                print(
                    "exp: cluster_controller.py startprocess dbmgr 123456789012345678 123 10.11.12.13")
                exit(1)
            ...
            clusterHandler = ClusterStartHandler(uid, componentName, machineip,
                                                 cid, gus, "", "", "")
        ...
        # python cluster_controller.py sendlog uid NORMAL|INFO|DEBUG|ERROR|WARNING logstr
            ...
            if len(sys.argv) < 4:
                print(
                    "syntax: cluster_controller.py sendlog [uid] NORMAL|INFO|DEBUG|ERROR|WARNING logstr")
                print(
                    "exp: cluster_controller.py sendlog 501 DEBUG \"send log test\"")
                exit(1)
            ...
            if len(sys.argv) < 3:
                ...
                if sys.argv[2].isdigit():
                    ...
                    print(
                        "syntax: cluster_controller.py save [uid] [save_file.cfg]")
                    print(
                        "exp: cluster_controller.py save 501 kbengine_process.cfg")
                    exit(1)
                filename = sys.argv[3]
            ...
            if len(sys.argv) < 3:
                ...
                print(
                    "exp: cluster_controller.py load 501 kbengine_process.cfg")
                exit(1)
            ...
            if len(sys.argv) == 3:
                ...
                if sys.argv[2].isdigit():
                    ...
                    print(
                        "syntax: cluster_controller.py load [uid] save_file.cfg")
                    print(
                        "exp: cluster_controller.py load 501 kbengine_process.cfg")
                    exit(1)
                filename = sys.argv[3]
